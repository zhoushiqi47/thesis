...
下面我们将障碍物的成像函数 $\hat I_d(z)$ 与前文中的点扩散函数 $\J_d(z,y)$ 联系起来。 有了前文中对 $\J_d(z,y)$ 相关分析结论，我们可以由此来分析半空间反弹性波散射问题的逆时偏移方法的分辨率。下面的定理， 告诉我们当采样点 $z$ 远离障碍物边界的时候， 成像函数在改点的值是非常小的。这种现象反映在后面的数值实验中， 就是在原理边界的时候， 在该处的颜色会非常浅。
...
\een
利用 (\ref{g2}) 我们得到 $u^s_q(x_r,x_s)\cdot e_i=\GG(u^s_q(\cdot,x_s),\N(\cdot,x_r)e_i), i=1,2$, 于是有
\ben
...
&=&\int_{\Ga_0^d}\sum^2_{i=1}[\T_D(x_r,z)]_{ij}\overline{T\N(x_r,x))^Te_i}ds(x_r)\\ \label{g6}
