...
&=&\frac 1{2\pi}\,{\rm p.v.}\int_L\sum_{\al=p,s}k_\al\,\frac{\Na(k_\al\sin t)}{\de(k_s\sin t)}\cos t\, e^{\i \lam_\al\cos (t-\phi)}dt.
\een
令 $L_{-\phi}$ 为 $L$ 平移 $-\phi$ 后的积分路径, 立即得到
\be\label{h1}
& &\frac{1}{2\pi}\,{\rm p.v.}\int_{\R}\hat \N(\xi,0;y_2) e^{\i(x_1-y_1)\xi} d\xi \\
&=&\frac 1{2\pi}\,{\rm p.v.}\int_{L_{-\phi}}\sum_{\al=p,s}\mathbb{N}_0^\al(t)\cos(t+\phi)e^{\i\lam_\al\cos t}dt,
\ee
...
\ben
& &\frac 1{2\pi}\,{\rm p.v.}\int_{L_{-\phi}}\mathbb{N}_0^s(t)\cos(t+\phi)\,e^{\i \lam_s\cos t}dt \\
&=&\lim_{\ep\to 0^+}\frac 1{2\pi}\int_{L^\ep_{-\phi}}\mathbb{N}_0^s(t)\cos (t+\phi)\,e^{\i \lam_s\cos t}dt\\
& &+\frac\i 2\sum_{t'=\pm t_R}{\rm Res}(\mathbb{N}_0^s(t)\cos (t+\phi)e^{\i \lam_s\cos t},t').
\een
\begin{figure}[htbp]
	\centering
...
\ben
& &\frac\i 2\sum_{t'=\pm t_R}{\rm Res}(\mathbb{N}_0^s(t)\cos (t+\phi)e^{\i \lam_s\cos t},t')\\
&=&\frac \i 2\sum_{\xi=\pm k_R}\frac{\Ns(\xi)}{\de'(\xi)}e^{\i (y_2\mu_s+(x_1-y_1)\xi)}.
\een
另一方面，通过 Cauchy 积分定理， 我们得到 
\ben
& &\frac 1{2\pi}\int_{L^\ep_{-\phi}}\mathbb{N}_0^s(t)\cos (t+\phi)\,e^{\i \lam_s\cos t}dt \\
&=&\frac 1{2\pi}\int_{L}\mathbb{N}_0^s(t)\cos (t+\phi)\,e^{\i \lam_s\cos t}dt.
\een
于是立即得到
\ben
& &\frac 1{2\pi}\,{\rm p.v.}\int_{L_{-\phi}}\mathbb{N}_0^s(t)\cos(t+\phi)\,e^{\i \lam_s\cos t}dt \\
...
同理可得
\ben
& &\frac 1{2\pi}\,{\rm p.v.}\int_{L_{-\phi}}\mathbb{N}_0^p(t)\cos(t+\phi)\,e^{\i \lam_p\cos t}dt \\
&=&\frac 1{2\pi}\int_{L}\mathbb{N}_0^p(t)\cos (t+\phi)\,e^{\i \lam_p\cos t}dt\\
& &+\frac \i 2\sum_{\xi=\pm k_R}\frac{\Np(\xi)}{\de'(\xi)}e^{\i (y_2\mu_p+(x_1-y_1)\xi)}.
\een
...
\ben
\frac 1{2\pi}\int_L\sum_{\al=p,s} k_\al\frac{\Na(k_\al\sin(t+\phi))}{\de(k_s\sin(t+\phi))}\cos(t+\phi)e^{\i\lam_\al\cos t}dt.
\een
由于， p 波和 s 波在表达形式上是相似的， 这里我们只分析含有 $[\Ns(k_s\sin(t+\phi))]_{22}=\mu^{-1}(2\xi^2\mu_p)(k_s\sin(t+\phi))$ 这项， 然后另外项的分析就同理可得.  为了表达简便， 我们有如下定义：
\ben
g(t)&=&k_s\frac{[\Ns(k_s\sin(t+\phi))]_{22}}{\delta(k_s\sin(t+\phi))}\\
&:=&f(t)(\kappa^2-\sin^2(t+\phi))^{1/2}, \\ f(t)&=&\frac{k_s^4}{\mu}\frac{(\sin(t+\phi))^2}{\de(k_s\sin(t+\phi))}.
\een
于是， 利用分部积分， 可以得到
\ben
...
\ben
& &\left|\int_{-\frac\pi 2}^{\frac \pi 2}g(t)\cos te^{\i\lam_s\cos t}dt\right|\\
&\le& C\lam_s^{-1/2}\left(|g(0)|+\int^{\frac\pi 2}_{-\frac\pi 2}|(g(t)\cos t)'|dt\right) \\
&\le& C\mu^{-1}\lam_s^{-1/2}.
\een
...
\ben
\left|\int_{L\bks [-\frac\pi 2,\frac\pi 2]}g(t)\cos te^{\i\lam_s\cos t}dt\right|\le C\mu^{-1}\lam_s^{-1}.
\een
于是得到 $|{\rm I}_1|\le C\mu^{-1}\lam^{-1/2}\cos\phi$ . 

 类似地，我们可以得到 $|{\rm I}_2|\le C\mu^{-1}\lam_s^{-3/2}$.  事实上，唯一的区别在于， 因为
\ben
...
\ben
\left|\int_{-\frac\pi 2}^{\frac \pi 2}g'(t)e^{\i\lam_s\cos t}dt\right|\le  C\mu^{-1}\lam_s^{-1/2}.
\een
于是， 我们最终得出
 \ben
...
如果当$\ep\to 0^+$ 时， $u_\ep$ 在加权 Sobolev 空间$H^{1,-s}(\R^2_+\bks\bar D)$, $s>1/2$ 中存在极限 $u\in H^{1,-s}(\R^2_+\bks\bar D)$. 显然，$u$ 满足如下方程：
\be
...
\een
 ...
 \begin{lem}\label{lap1}
 	...
 	\ben
 	\lim_{\ep\to 0} \|u_\ep-u\|_{H^{1,-s}(\R^2_+\bks\bar D)}=0,
 	\een 
