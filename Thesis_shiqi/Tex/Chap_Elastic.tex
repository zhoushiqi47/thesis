...
对于任意 $t\geq 1$, 易得：
\ben
f'(t)&=&8t-2-4\sqrt{t-1}\sqrt{t-\kappa^2}-2t\sqrt{t-1}/\sqrt{t-\kappa^2}-2t\sqrt{t-\kappa^2}/\sqrt{t-1}\\
&\leq& 8t -2 -4\sqrt{t-1}\sqrt{t-1}-4t =-2
\een
又因为 $f(1)=1$ 以及 $f((2-\kappa^2)/(1-\kappa^2))<0$ 。 于是立得 $k_R^2\le\frac{2-\kappa^2}{1-\kappa^2}k_s^2$ 。 然后由中值定理及 $f'(t)\leq -2$ 得
\ben 
\min_{|\xi|\ge k_s}|\frac{\de(\xi)}{\xi^2-k_R^2}|\ge\min_{t\geq1}|f'(t)|k_s^2\ge 2k_s^2,
\een
结论2， 3得证。
\finproof
...
\een
于是， 利用分部积分就可以得到在 $-\pi/2+\i\infty\to -\pi/2$ 积分值关于 $\lambda$ 是一阶衰减的。 同样利用分布积分也可以得到在区间 $\pi/2\to\pi/2-\i\infty$ 上的积分值的估计。 于是， 不等式中的第一项估计得证。 类似地， 可以证明不等式中的第二项的估计。 引理得证。
\finproof
